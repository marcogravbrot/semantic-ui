		...
		for k, v in pairs(var) do
			for _, w in pairs(v) do
				for k, v in pairs(w) do
					table.insert(tbl, {Type = k, Data = v})
				end
			end
		end

		panel:AppendLine(unpack(tbl))
	end,
}
...
function PANEL:StartInsertion()
	self.CurFont	= "Lato Regular"
	self.CurColor	= semantic.color.text
...
semantic.register( "text", PANEL, "Panel" )